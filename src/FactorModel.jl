...
struct FactorModel
    ...
    function FactorModel(x, number_of_factors::Int64=ceil(Int, minimum(size(x))/2), number_of_factors_criterion::String="", factor_type::String="principal components")
    ...
    function FactorModel(x::Matrix{Float64}, number_of_factors_criterion::String, factor_type::String="principal components"; max_factors = ceil(Int, minimum(size(x))/2))
        ...
        models = [FactorModel(x, number_of_factors, number_of_factors_criterion, factor_type) for number_of_factors in 1:max_factors]
        ...
        return models[argmin(criteria[1:max_factors])]  # keep the model with the best information criterion
    end
...
function calculate_factors(x::Matrix, factor_type::String="principal components", number_of_factors=ceil(minimum(size(x))/2))
    if factor_type == "principal components"
        ...
        max_factor_number = ceil(Int, minimum(size(x))/2)
...
function Base.show(io::IO, fm::FactorModel)
    ...
    @printf io "Factor model residual variance: %s\n" sum(fm.factor_residuals.^2)/reduce(*, size(fm.x))
end
