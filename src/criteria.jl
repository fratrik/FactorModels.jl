factor_residual_variance(fm::FactorModel) = sum(fm.factor_residuals.^2)/reduce(*, size(fm.x))  # see page 201 of Bai Ng 2002
...
function criterion_PCp2(fm::FactorModel; kmax=8)
    ...
    N_plus_T_by_NT = reduce(+, size(fm.x))/reduce(*, size(fm.x))
